...
config = {'CACHE_TYPE': 'SimpleCache','CACHE_DEFAULT_TIMEOUT':86400000}
    ...
    if request.method == 'GET':
        ...
        if upload_file != '':
            try:
                ...
                path = os.path.join(app.config['UPLOAD_FOLDER'],f'{str(uuid.uuid4())}.webp')
...
def update():
    ...
    metadata.query.filter_by(id=id).update(dict(title=title if title !='' else photo_data.title,sub_title=sub_title if sub_title !='' else photo_data.sub_title,category=category if category !='' else photo_data.category, photo_path=path.replace('static/','') if upload_file != b'' else photo_data.photo_path))
    db.session.commit()
